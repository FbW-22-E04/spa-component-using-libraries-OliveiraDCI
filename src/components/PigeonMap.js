...
export default function PigeonMap({ foundProviders, searchTerm }) {
  const [center, setCenter] = useState([
    48.949208441689805, 12.304167000000007,
  ]);
  ...
  return (
    ...
    >
      <ZoomControl />
      {foundProviders.map((provider, index) => (
        <Marker
          key={index}
          width={50}
          anchor={[provider.location.latitude, provider.location.longitude]}
          payload={provider.href}
      ...
      {markerContent && (
        <Overlay
          ...
          anchor={[
            markerContent.location.latitude,
            markerContent.location.longitude,
          ]}
        >
          <h1>{markerContent.name}</h1>
          <ul>
            {markerContent.company.map((item, index) => (
